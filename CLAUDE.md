...
| `docs/design/mockups/DEMO_PROGRAM.md` | **How rules, demos and sessions work from 2026-09-08:** the three rule tiers (always / by tag / at test), the demo-candidates session, the hub's categories, the session order with Andrew's opening lines, the adopted model policy. Its session briefs sit beside it: `OPUS_FIX_PLAN.md` (the fixes), `FABLE_CONTROL_BRIEF.md` (the control build and its review). `docs/design/PRE_BUILD_TODO.md` is the list of things to do before the master build (P-01: the experience-balance brainstorm). |
| `docs/design/mockups/SHOWCASE_BRIEF.md` | **The showcase lane** (branch `phase-0.75-showcase`, worktree `StewartSquad-showcase/`, :5184; `docs/LANES.md`): five demos built to be looked at (the Crystal Colossus, a Rootways room, the biome hand-off, the Volcanic Rift, the camp's growth stages) under `sandbox/showcase/`, quarantined by the brief's §1 and left by one of its §2 exits. Nothing there is canon, a rule or a decision by existing; its own log and findings are `SHOWCASE_LOG.md` and `SHOWCASE_FINDINGS.md`. |
