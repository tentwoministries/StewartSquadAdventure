...
## ui-ux.md — 941 lines — PASS

| # | Result |
|---|---|
| 1 | Pass. Surface inventory (49 surfaces) and typeface table open §2. |
| 2 | Pass. Every FAMILY_CANON §12 string is placed verbatim or listed as cut with a story-beats reason (§2.5.1–2.5.2); spot-checked the journal footer with its two canon spaces, `💰 Drag here to sell`, `Choose an upgrade (click or press 1/2/3)`, `Host is choosing — click to vote!`, `Save before returning to title?`, `Achievement Unlocked!`, `The Stewart Squad has fallen...` — verbatim. New strings collected in one table (§2.5.3) across the whole bible, all marked. The crest is props and places only (no faces, initials, motto). No §2(b) question. |
| 3 | Pass. §3.1 keeps AR §17's warm-on-cool identity as the HUD's palette and the biome retint as the accent rule, replaces the glass keyframes with one page-turn, keeps the letterbox and damage-number recipes, and keeps the HUD out of the world's layers (corner-anchored, idle fade, 26 % of the frame left as living world beside the book). |
| 4 | Pass. Layouts at 1920 × 1080 and 390 × 844 with sizes; beams, nameplates, HP bars and damage numbers are in-scene billboards with distance fades; the boss bar adopts bosses.md §2.4 whole. |
| 5 | Pass. `ui.*` tokens with an explicit anti-palette check (no pure black, no neon, paper at 9.3:1 contrast); two v27 hexes retuned (`#ff0000` → `#E8362A`, `#00d2ff` → `#3AF0FF`) with the strings unchanged. Typefaces Nunito + Lora + a Noto Emoji monochrome subset, all SIL OFL, bundled woff2, a Rule 2 licence gate named. |
| 6 | Pass. Zero UI lights; in-scene text in two instanced draws; beams ≤ 12 instanced; no `backdrop-filter` below High; ≤ 2 ms main-thread layout with a page open (measured). |
| 7 | Pass. Page anatomy, the page set and tab order, HUD layouts, the prompt template that reproduces the canon `[SPACE] …` strings under default bindings, full input maps, remapping rules, a settings table with defaults, onboarding with pad/touch variants, save export/import, the crest's four levels, the dev console as the `window.__ssq` harness contract. |
| 8 | Eight items designed around. Accepted rulings: the emote wheel moves to View-hold on pad (a heroes.md addendum cell); camera yaw on `Z` / `C` and right-drag because `Q` / `E` are canon combat binds (the Brief §4.5 suggestion yields to canon; the bible is more specific than the brief here); the Queen's bar text darkens only during a kidnap on Collette; the loading memory is a cached capture; `NPC_DEFS` icons render as tinted Noto glyphs; canon all-caps strings keep their case. **Consistency-pass edits (dungeons.md and cutscenes.md landed after this file's draft):** (a) the dungeon HUD — dungeons.md's compass **room strip** is the HUD element; ui-ux's "dungeon paper minimap" moves to the map page's dungeon view (rooms as paper cells inked when visited) toggled by the canon `Minimap` setting; the room-transition iris is cut (dungeons: contiguous rooms, no fade); the mechanic-meter slot stays but dungeons uses it only for `Channeling... (N more)`; (b) caption contract — adopt cutscenes.md §2.2.5 (portrait 96 / 64 px, text 20 / 17 px `#EDE3CF`, the `clamp(chars × 0.06, 2.2, 5.5)` hold); (c) the skip prompt — 13 px `#EDE3CF` at 45 %, armed at 1.0 s, per cutscenes.md §2.2.7; (d) photo-mode camera limits — confirmed by cutscenes.md §2.13 (pitch 5–80°, distance 2–30 m, FOV 24–50°, no roll; `Tilt` maps to the aperture control). |
| 9 | 33 lines merged into `DECISIONS.md`. |
| 10 | Empty. |

## audio.md — 1,143 lines — PASS (reviewed 2026-09-07; reconciled to ui-ux.md, dungeons.md, cutscenes.md)

| # | Result |
|---|---|
| 1 | Pass. The v27 bootstrap, the dispatcher, all 29 recipes by synthesis, the call-site counts, BGM9, replication and the mix open §1 with line numbers; the three undefined names (`shield`, `heal`, `questComplete`) verified against the HTML. |
| 2 | Pass. `Music: OFF` / `Music: ON`, `M: Mute`, `#6B8EC8` and the 1.5 s, `Bounty complete: <name>!`, `Aegis activated!`, `<name> revived by Phoenix Feather!`, every boss phase string quoted in §2.2.4, `A mysterious merchant has appeared!`, `A mysterious portal appears...` and `Grandpa Ed says this kind of rain is 'character-building weather.'` grep-verified in the v27 HTML. `It got away.` is world-events' new text and is now marked as such. |
| 3 | Pass. AR §19.1's shared torch oscillator becomes the crackle family's gain follower (light, flame and sound agree); AR §19.2's five-layer minimum is the ambience rule; AR §19.3's four incommensurate sources are the engine recipe; AR §13.1's intro (growl at 0.8 s at P0, music to silence at 0.0 s) kept exactly; AU §18's meteor duck verbatim; the 29 recipes port value for value under `snd()`'s 0.25 default. |
| 4 | Pass. Listener at the active hero with the camera's yaw (the 19–22 m camera argued through); five attenuation classes; one-shots panned once at spawn, loops on 20 Hz panners; the plane heard 60–90 m out before its shadow blob shows. |
| 5 | Pass. No colour is decided here (heroes.md's); the mix bands are AU §24's under a limiter, so the v27 boss-death double hit no longer clips. |
| 6 | Pass. Voice pools (32 / 24 one-shots, 8 loops, 8 bed slots, 6 chirps), P0–P6 stealing, per-name throttles, three noise buffers generated at boot, no worklet and no convolver, ≤ 0.3 ms of main thread per frame; mobile caps named. |
| 7 | Pass. Seventeen base recipes in the AUDIO_INVENTORY style with their parameters; the cue map as a typed table; test hooks against `OfflineAudioContext` including the transcription test and a grep-driven coverage test; Rule 2 gates listed. |
| 8 | Eight items designed around, all accepted. Orchestrator additions 2026-09-07 (the file was written concurrently with `ui-ux.md`, `dungeons.md` and `cutscenes.md` and landed last): their cue names win, and §2.2.8 and §2.5.6 were rebuilt to them (`ui.*` / `card.*` / `photo.shutter`; `dng.*`, `grove.*`, `tomb.*`, `temple.*`, `ice.*`, `shard.*` and the `amb.dng.*` keys including `shard.f1–f3` and `hearth`; `cs.*`) with the rename ledger in §5.3 item 8; settings take ui-ux's labels and 0–100 % sliders (§5.3 item 9); the `ui.*` prefix rule with the token-keyed announce stingers as the one overlap (§5.3 item 10); cutscene ducking takes cutscenes.md §2.14's numbers; the crystal-resonance notes are dungeons.md's C4 E4 G4 A4 C5. One prompt error of mine: the brief told the agent the weather crossfade was 3 s; the agent correctly followed world-events' 6 s (§5.3 item 4). |
| 9 | 39 lines merged into `DECISIONS.md` (34 from the review, 5 from the reconciliation). |
| 10 | Empty. |
