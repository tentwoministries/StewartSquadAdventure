...
| 8 | The wreck (The Green Meanie, `wrecked`) | nose at (11.5, 0.4), tail at (17.6, −0.4) | **6.0 long, 7.2 span, 2.5 tall** (`npcs.md` §2.2.1, confirmed; the assumed ≤ 6.5 × 8.0 × 2.6 m envelope is superseded, and the hangar does not grow: 7.2 m of span leaves 0.9 m each side of the 9 m opening and 2.5 m of aircraft leaves 1.1 m under the 3.6 m clear height); nose 0.6 m into a 1.6 m dirt mound; lower-left wing snapped, tip on the ground at (13.0, 4.2); no propeller; tail up 0.4 on the skid | 262° | `npcs.md` mesh (GLB or procedural) · ≤ 3,500; the mound, mesh damage and dirt are this file's | livery per `npcs.md`; mound `camp.earth` | a loose fabric strip on the broken wing (cloth), a dangling wire (1-bone, 1.1 rad/s), engine smoke wisps 0.1/s (C0: 1.0/s) | — | a biplane on its nose beside the tent |
...
**Ed's spots are `npcs.md`'s, not this file's** (consistency pass 2026-09-07: `npcs.md` §2.1.6 and §2.3.1 own them under the rule "Ed is wherever the plane is", and the earlier coordinates stated here are superseded). Repeated here so the props are placed under them: the **wreck's nose (15, −2.5)**, facing the engine, for C0–C3 (the `crash_landing` lines, greetings and the B1.2 talk); the **Forest Ed mark (20, −2.2)**, beside the hangar's mouth, from C4 on; the plane's **parking mark (16, 0)** with its **nose at (19, 0)** and the **cockpit step (16, −0.9)** where the `Fly` prompt lives; the **apron** beside the plane for turn-ins, CS-05 and supply runs; **Ed's stump (1.9, −0.5)** at the fire whenever he sits there (from `portalOpen` on, and in the ending); and the crater rim for CS-06 (`story-beats.md`). Build the hangar, the strip marks and the stump to these; if a prop and a mark disagree, the mark wins. The merchant is this file's: he stands at (9.5, −6.2) in front of his cart facing the fire. The biome guides never visit; the bounty board also stands at each island hub (`story-beats.md` §2.8) as the same prop.
...
| Fliers | `enemies.md` §2.2 "Flight" (reconciled 2026-09-06) flies the Bat at 0.9 m and the Ember Sprite at 0.6–1.2 m and lets them clear ground props only to **0.5 m**, so the 1.1 m fence **blocks** them; the camp's enemy exclusion (`enemies.md` §5: no camp or nest within 30 m, and a 40 m leash from home) makes it moot in any case |
...
**The same prompt runs at dungeon hearths** (`dungeons.md` §2.1.6, confirmed in the 2026-09-07 consistency pass). Each dungeon's entry hall and its one mid-run rest room offer this prompt verbatim — `Rest` with `Until dawn` / `Until dusk`, and `Sit` (§2.7.4) — on the same no-enemy-within-20 m gate, healing to full, clearing DoTs and advancing the clock with the same 3 s sky sweep. The prop there is this file's fire rebuilt as a 0.9 m stone ring the family lights on entering (a 0.6 s kneel by the active hero, `light.campfire` at the token values, one pooled light, the shared oscillator), and it stays behind them, cold, on re-entry. **One bespoke variant:** the **Hermit's stove** in the Ice Citadel's Quarters, a stove with a kettle on it, whose prompt is the one a kid will use on purpose (`Until dusk`, because the Citadel's lift wants the aurora). Home, Wrong is still the exception: the mirror fire offers nothing (§2.7.6).
...
CS-10's last shot (`cutscenes.md` §2.11 shot 12) is S5 with C7 — and, corrected in the 2026-09-07 consistency pass, it holds **four kids in their seats with Ed's stump empty**, exactly like the title scene above, because **Ed flies the last pass** (`npcs.md` §5.3 item 4; `cutscenes.md` §5.2's row for this file). "All five at the fire" is true of the ending's earlier shots (5–9); Ed walks to the plane at shot 9 and is airborne from shot 10. The fox is on its bed, and the plane is over the Ridge.
...
| File | Must pick up |
|---|---|
| **`npcs.md`** | **Ed's spots are `npcs.md`'s** (§2.1.6, §2.3.1), and §2.3.3 now defers to them: the wreck's nose (15, −2.5) C0–C3, the Forest Ed mark (20, −2.2) from C4, the parking mark (16, 0), the nose (19, 0), the cockpit step (16, −0.9), the apron for turn-ins, and this file's stump (1.9, −0.5) when he sits; the merchant's spot (9.5, −6.2) at his cart and the cut relocation; the guides never visit; the plane's envelope is **confirmed** at 6.0 × 7.2 × 2.5 m against the hangar's 9 m × 3.6 m clear opening, so the hangar does not grow (§2.2 #8); the `wrecked` mesh at the furrow's west end and the `hopping` swap at C4's 17 s; the build clips `ed_hammer`, `ed_saw`, `ed_hoist`, an `ed_sit` on the stump and whittling from C4; the spare goggles prop on the scarecrow; the windsock as the plane's wind reference; supply drops within 60 m of the strip but never inside the C6 footprint except the strip and apron; the plane parked on the apron (24, 0) from C7; the title's plane pass over the Ridge at 18 m; Ed's greetings at the fire |
...
| **`world-events-weather.md` (consistency pass — all three applied 2026-09-07)** | the below-rim clouds are visible only from a camera within about 24 m of a rim at gameplay pitch (§2.11.5) → now in its §2.6 clouds row; the two lantern value sets (§5.3 item 2) → its §2.8.2 `light.lantern` row now carries both, and the camp's three are 1.2 · 6 m; the axis sentence (§5.3 item 3) → its §2.1.2 now says north is `−z` |
...
2. **`world-events-weather.md` gives two values for a placed lantern:** §2.1.3's camp lanterns `#FFB347` · 1.2 · 6 m and §2.8.2's `light.lantern` 1.0 · 4 m placed. This file uses §2.1.3's values for the three pooled camp lanterns (they are the explicit camp schedule) and asked the consistency pass to make §2.8.2 say so. **Applied 2026-09-07:** its `light.lantern` row now carries both placed values — 1.0 · 4 m for dungeon and post lanterns, 1.2 · 6 m for the three camp lanterns.
3. **Axis convention.** `story-beats.md` fixes `+z` south (final); `world-events-weather.md` §2.1.2 says "the island's north is +Z". Its azimuths (dawn 80° east, noon 180° south, golden hour 245°, moon 130° at night) are read here as compass bearings in the `story-beats.md` frame with north = `−z`; every station's sun and moon direction in §2.11.5 assumes that. **Applied 2026-09-07:** its §2.1.2 now states north = `−z` and reads the azimuths as compass bearings in story-beats' frame, so nothing in §2.11.5 moves.
...
None. No Brief §2(b) portrayal question arose: the pet names assert nothing about real people and are data; bedrolls are unassigned; the four painted stones carry initials the canon already scratches into rocks; Ed's stump, bench and cot are props, not claims about how he lives. No Rule 1 input was missing for this system (the plane's dimensions were a stated assumption for `npcs.md`, not a blocker; confirmed at 6.0 × 7.2 × 2.5 m in the 2026-09-07 consistency pass, §2.2 #8).
