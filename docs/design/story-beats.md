...
| 3 · B3.5 | Lights in the dark | the real crater, then Stewart Camp C7, dawn to golden hour | Queen dead, victory stats continued | **CS-10** ending (about 50 s) | all four, Ed, the deer | `SHADOW QUEEN VANQUISHED!` / `The darkness has been defeated!` (stats); `VICTORY!` / `The Stewart Squad saved the realm!`; closing card **[new text]**; victory buttons (§12.3) |
...
The one light: throughout Home, Wrong, a single warm light source exists that is not the player's — the real camp fire, seen through the void as a far warm glow on the horizon in the direction of the real world (a directional fill at 4 % intensity, colour `#FF9A3C`, plus a horizon billboard). It never lights the ground enough to see by. It is the title of the game made literal, and it is the lighting brief for `dungeons.md` and the "best lighting in the game" the Brief §5.3 asks for.
...
| CS-03 | The meteor | B2.2 | 24.2 s | yes | seven canon scenes reshot; scene 1 from Liam at the fire, Ed in frame; impact framed on the crater from camp | `meteorSeen`, `craterDiscovered` |
...
| CS-10 | Lights in the dark | B3.5 | about 50 s | yes | the real crater at dawn, the four walking home, the camp at C7 in golden hour, the deer, the plane over the Ridge, the title over the tagline | camp C7 |
| CS-11 | NG+ opener | B4.1 | 40 s | yes | CS-01 without the ambush | as CS-01 plus `sibsRescued = 3` |
...
| `audio.md` | CS ids and beats as cue anchors; the plane-state table for the engine sound's health; the shard as a night ambient source |
