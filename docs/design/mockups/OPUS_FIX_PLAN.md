...
The Opus 5 experiment is judged (`docs/design/mockups/OPUS_EXPERIMENT_VERDICT.md` on branch `phase-0.75-opus-experiment`, worktree `StewartSquad-opus/`; read §0, §3 and §8 of it first, and `docs/visual-loop/opus-experiment-scores-2026-09-08.md` for the art-director's per-frame change lists). This session brings the keepers into the reel, fixes what the review found, turns the review's method into code and tests, re-shoots the hero frames, and retires the experiment lane. Fable 5.1 at **high** plans, reviews and commits; `sandbox-builder` agents (Opus, high) do the building, as the run table below says (this session is the pilot of the delegated loop, and supersedes the "orchestrator writes the sandbox directly" exception for anything an agent can be briefed on). Shared files under `sandbox/_shared/` may be edited in this session by the one agent the table assigns, with the standing rule: a change there is checked with one frame per scene.
