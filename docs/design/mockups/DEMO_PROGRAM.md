...
3. **`misc` is a real tag, and it is cleaned up on a schedule.** A lesson that fits no category is tagged `misc` rather than forced into one; when three or four `misc` rows are about the same thing, the next tidy promotes them into a new tag and renames them (Andrew, 2026-09-08). The tidy happens at every application step and at the full-ledger review before Phase 1 (`docs/design/PRE_BUILD_TODO.md` P-09).
4. **A row has a status, and only `fixed` rows are rules.** Something Andrew notices in a demo (the caves' stairs meeting the tiers at their insertion points, the kids clipping into rock on the way down: T-34) is logged as a tweak row, tagged, with status `observed`. It becomes a ledger row only when it is fixed, because the rule is the fix, not the symptom. Until then it is a to-do, not a constraint on anyone's first pass.
5. **Tags are the per-task net; the whole ledger is still read, at gates.** A rule that applies but was not tagged with the task's tags is the residual risk of Tier 1. It is covered where it is cheap: the `qa-inspector` reads the full ledger against the delivery at every phase gate (checklist P0 and P4 already say so), and Tier 2's checks run on every delivery regardless of tags. Per task: tags and checks. Per gate: the master list, once. That is the whole of it; nothing else is added.
6. **Brief hygiene.** A demo or build task is one page: what, why, constraints, the tag line, the acceptance checks. Creative decisions stay the builder's (Brief §2). The rules arrive through the tags and the checks, not through the prose.
