...
| Dungeon room strip | HUD (under the compass) | in a room dungeon | no | dungeons.md §2.1.10 (spec), this file (render) |
| Dungeon overlay on the map page | page | the map page, in a room dungeon | yes | this file; the canon `Minimap` setting toggles it |
...
| Top-right | margin 24, 320 × 28 strip, chips below | margin 12, 200 × 22 strip | compass strip (§2.4.5), the dungeon room strip directly under it in a dungeon (§2.4.13), chips (§2.4.4), quest tracker (§2.4.6), mechanic meter (§2.4.13) |
...
| Bottom-right | margin 24 | touch cluster | achievement toasts (§2.4.10); on touch, the action cluster; photo prompt |
...
**Speech bubbles** (proximity lines, emotes, caged lines, party lines): world-anchored billboards 0.4 m over the head, `ui.night` at 85 % with a 1.5 px border in the speaker's colour, Nunito 600 14 px, max 34 characters per line, 5 s, fade 0.3 s, never more than three on screen (oldest goes). **Captions** (cutscenes, in-flight lines, the sheltering tip): two styles, both DOM in the letterbox layer and never in the HUD, adopting `cutscenes.md` §2.2.5 whole (2026-09-07). **line** — a character speaks: bottom, 24 px above the bottom bar; the speaker's portrait at 96 px desktop / 64 px mobile from the `npcs.md` §2.1.7 atlas at the mood's frame, in a ring in the speaker's colour; the name in the speaker's colour (Ed and NPCs their `ui` token, the kids `hero.<kid>.glow`) at 16 px, then the text on the same baseline in `#EDE3CF` (`camp.cream`) at 20 px desktop / 17 px mobile, **one line at a maximum of 60 % of frame width** (it wraps to two on a phone and is still one caption); typewriter at **120 cps** with one `dlg.blip.<who>` per glyph group; on screen for **`clamp(chars × 0.06, 2.2, 5.5)` s** from the first character; 0.3 s fade; captions never overlap and leave a **≥ 0.2 s gap**. **announce** — a world string a cutscene shows: centred, **18 % from the top** under the top bar, on a `#0B0E1A` pill at 60 % alpha, **22 px in the string's own v27 colour** for the string's v27 duration. The sheltering caption `Grandpa Ed says this kind of rain is 'character-building weather.'` (canon tip 2) uses the **line** style with no portrait (world-events §2.7.3). The `Subtitles` setting (§2.7) gates captions only, never dialogue.
...
| Room transition | **No fade** (2026-09-07). `dungeons.md` §2.1.3 makes rooms contiguous — a doorway is a real 2 m gap in a wall and the party walks through it — so v27's room-slide *and* this file's 0.3 s iris are both cut; a fade between two rooms you can see into at once would be a lie. `Room N/M` (canon) refreshes on the room strip's title as the active hero crosses the threshold plane. The only fades in a dungeon are the threshold (the intro grammar), the exit (`exitDungeon`'s 1 s), the shard's region banner gates, and the hearth's door |
| **Room strip** | The dungeon map, on the compass (2026-09-07; `dungeons.md` §2.1.10 owns the spec, this file the render). Directly under the compass band, top-right, 220 × 22 / 160 × 18 px on a `ui.night` band: the **critical path** as a row of 8 px pips at a 14 px pitch, **optional rooms** as 5 px ticks hanging off their parent pip, **visited** pips filled in `ui.cream`, **cleared** pips carrying a `✓`, the **current** pip pulsing `0.85 + 0.15·sin(2πt·0.8)` in `ui.accentGlow`, the canon `🔒` glyph drawn between two pips where the door is locked, and `🔑 N` (canon) at the strip's end. Title `Room N/M` (canon) in Nunito 700 11 px above it. Always on: it is part of the compass, not the minimap, so the canon `Minimap` setting does not gate it and `room.visOverride` never dims it |
| Dungeon overlay on the map page | The paper map (§2.3.5) gains a dungeon view while a room dungeon is running: rooms as `ui.paper` cells inked as they are visited, connections drawn in `ui.pencil`, and the same glyphs as the strip (`🔒` on a locked connection, `✓` on a cleared room, a crown on the boss room once seen), with `Room N/M` and `🔑 N` as its title. **This is what the canon `Minimap` setting toggles** (§2.7). It is a page and never a HUD corner, so the strip and the paper map are never on screen together |
...
| **Mechanic meter** | one slot, 220 × 10 / 160 × 8 px under the chips, fed by `dungeon.meter { label, value, max, colour, icon, state: 'idle' \| 'rising' \| 'falling' \| 'danger' \| 'done' }`: a `ui.night` trough, a fill in `colour`, a 16 px sprite icon at the left, the label in Nunito 700 12 px, `danger` pulses the rim `ui.hurt` at 2 Hz, `done` stamps a `✓`. **2026-09-07:** `dungeons.md` §2.1.10 names **no** per-dungeon meters — the lantern's light, the sand level, the rope, the aurora charge and the root rhythm are each read in the world (a shrinking ground glow, a lit glyph on a wall column, a hand on a rope, a conduit that glows the sky's colour) — so the slot serves `Channeling... (N more)` and any meter a later §6 line adds. The slot stays: it costs nothing while empty, and it is cheaper to keep than to re-derive |
...
| Lairs | the Depths and the Rift use the entry card with `The Crystal Depths` / `The Volcanic Rift` and their canon descriptions, no room strip and no map overlay (they are places, not room sets), the meter if bosses.md's fight sets one |
...
| dungeons.md | the item `Rusted Lantern` and its card line `It won't stay lit outside. Inside, it's the only thing that does.`; the checkpoint prompt `Resume <name> from the hearth?`; the two fail-card actions `Try again` and `Leave`; the room and mechanism names, used as build ids and as the quiet name cards for rest and set-piece rooms (§2.4.13; listed per dungeon there) |
| cutscenes.md | the pad skip variants composed from the live binding (`Press A to skip`, `Press ▢ to skip`), `Tap to skip` on touch, and the multiplayer vote suffix ` · N/M` |
| audio.md | `Dialogue blips` (the settings toggle; §2.7) |
...
| Play | `Minimap` | ON / OFF (canon) | the map page's **dungeon overlay** (§2.4.13); the compass room strip is the dungeon HUD element and is not gated by this |
...
| Hear | `Master`, `Music`, `Effects`, `Ambient`, `Dialogue` **[new text]** | sliders 0–100 %; defaults **80 / 60 / 80 / 70 / 70** | the five buses `audio.md` §2.8.2 exposes, under this file's labels; 60 % on `Music` is v27's `bgm.vol` 0.15 through audio.md's curve |
| Hear | `Dialogue blips` **[new text]** | ON / OFF, default ON | turns the typewriter blips (`dlg.blip.<who>`) off in the dialogue box and in captions; the advance cue `dlg.advance` still plays (`audio.md` §2.8.2 — some players find blips wearing) |
| Hear | `M` mute (canon) | the canon announce `Music: OFF` / `Music: ON` | mutes the `Music` bus without moving its slider; persisted separately from the slider so unmuting restores the level |
...
**Audio persistence (2026-09-07).** The seven `Hear` settings persist as `audio.master`, `audio.music`, `audio.effects`, `audio.ambience`, `audio.dialogue`, `audio.dialogueBlips` and `audio.musicMuted`, in the settings schema (`ssq_settings`) beside v27's eight keys and **never in a save slot** — a kid changing the volume must not touch a save. The keys are `audio.md`'s; the labels and the 0–100 % range are this file's, and `audio.md` adopts them.
...
| `Focus` **[new text]** | **the focus distance.** Tap or click a point to focus on it; `Auto` (the target's depth) is the default, and a slider sets it by hand from **1 to 30 m** (`cutscenes.md` §2.13) |
| `Tilt` **[new text]** | **the aperture control.** 0–100 % maps `cutscenes.md` §2.13's aperture 0–1, which narrows the tilt-shift band from **60 % to 15 %** of frame height and raises max blur from **0 to 4 px** — photo mode is the one place the diorama's tilt-shift becomes a real depth of field. Default: the island's tilt-shift strength |
...
Camera: orbit within cutscenes.md's limits by the same inputs as play (RS / drag / `Z` `C`), zoom by wheel or `L3`; `Esc` / `B` leaves photo mode and restores the HUD and sim. Reduced motion changes nothing here except the flash (off) and the orbit, which becomes direct with no inertia.

**The limits, confirmed by `cutscenes.md` §2.13 (2026-09-07)**, so this file no longer assumes them: yaw free 360°; **pitch 5°–80°** (5° so the camera never dips under the ground plane, 80° so a straight-down diorama shot exists); **distance 2–30 m**, default 12 m; **FOV 24–50°**, default 35°; height offset ±1.5 m on the target; **roll always 0** (a rolled diorama is a broken toy); collision by a 0.3 m sphere cast that stops the camera at geometry; **focus 1–30 m** (`Focus`) with the aperture mapping the tilt-shift band 60 % → 15 % (`Tilt`); the target cycles heroes, Ed, the fire, any pet or animal within 10 m, and the plane. Time of day may be overridden (a seven-stop selector of the keyframes, cosmetic, blended over 1.0 s, the clock unmoved, released on exit); **weather may not** — weather is state, and rain that stops for a photo is a lie the world tells. Exit blends 0.5 s back to the gameplay rig.
...
| File | Must pick up |
|---|---|
| **audio.md** | **The contract, both ways (2026-09-07).** *To audio.md:* this file's cue names and setting labels are the ones `src/content/audio/cues.ts` and the settings page use — the seven `Hear` settings under the labels `Master`, `Music`, `Effects`, `Ambient`, `Dialogue`, `Dialogue blips` and the canon `M` mute (`Music: OFF` / `ON`), at the ranges and defaults in §2.7. *From audio.md:* its **set** of settings and its **duck values** win. The portrait strip plays `hero.swap.<hero>` (v27's `hit` 0.15 on a swap is gone, as is `heal` 0.2 on the post-death auto-switch), pitched in age order, oldest lowest; the emote wheel plays `hero.emote.<hero>` (three of that hero's dialogue blips); every location, camp, dungeon-entry and constellation card plays the quiet title-card swell; the level-up card and the scrapbook pages duck music −3 dB under a 2 kHz lowpass (heard through paper) and ambience −6 dB; the pause menu ducks music −6 dB and ambience −9 dB, both under a 1.2 kHz lowpass; the **announce channel is silent by itself** (each announce carries its own cue, where it has one); the dialogue log page makes no sound; the catch card plays `fish.catch`; the shutter is this file's `photo.shutter`. **Namespace rule (orchestrator decision):** the `ui.*` prefix is shared by the style tokens in `src/style/ui.ts` and the cue table (`CueName` in `src/content/audio/cues.ts`); no member name may appear in both, with one deliberate exception: the announce stingers `ui.announce.hurt` / `ui.announce.alarm` are looked up by the announce's colour token and so share its name (`audio.md` §2.2.8); a test asserts the two key sets are disjoint outside the `ui.announce.` prefix. UI cues by name: `ui.page` (the page-turn), `ui.open` / `ui.close` (the book), `ui.tab`, `ui.focus` (a soft tick on focus moves, pad and keyboard only), `ui.confirm` (= v27 `equip` 0.3), `ui.cancel`, `ui.error` (v27 had none: a short low double-tap for `Not enough gold`, stash full, wrong hero), `ui.toast` (= `achieve` 0.3), `ui.announce.<colourToken>` (optional stingers; `hurt` and `alarm` get one), `ui.prompt.show`, `ui.swap` (heroes.md's chime), `ui.wheel.open` / `.pick`, `ui.radial.open` / `.pick`, `ui.cooldownReady` (once per icon), `ui.lowHp` (the ring's pulse), `ui.tutorial.step` (= `achieve` 0.15), `ui.save`, `ui.load`, `ui.export`, `photo.shutter`, `dlg.blip.<who>`, `dlg.advance`, `dlg.skip`, `card.title` (a soft chord on a title card), `card.catch`, `card.constellation`; the bounty-complete cue camp.md and npcs.md asked for (`questComplete`); silence rules: no sound on idle fade, on chip changes, on compass pips |
| **cutscenes.md** | Aligned to that file 2026-09-07; it landed after this file's draft and owns the cinematic contract. The **caption contract** `{ speaker, portraitFrame, text, t0, t1 }` rendered by §2.4.11's two caption styles (`line` and `announce`, adopted whole from its §2.2.5), gated by `Subtitles`. The **letterbox** as one component with **two presets**: `bars.cinema` at 5.6 % of frame height (60 px at 1080p, 0.5 s in ease-out / 0.4 s out ease-in) for every cutscene and every boss intro, and `bars.punch` at 12 % (0.15 s in / 0.5 s out) for the combo beat, both in `cs.bar` `#0B0E1A`; **layer order** bars `z` 55 → captions and the skip prompt `z` 56 → cards (name, title, skip) `z` 57 → CS-06's dialogue box `z` 58, with no HUD drawn while bars are up. The **skip prompt** at **13 px `#EDE3CF` at 45 % alpha, centred 8 px above the bottom bar, armed at 1.0 s** and only on a fresh press (a key held from `START ADVENTURE` cannot skip CS-01): `Press SPACE to skip` (canon) as the keyboard variant, `Press ▢ to skip` on pad, `Tap to skip` on touch, and the ` · N/M` vote suffix while a multiplayer skip is pending; never on the boss intro or the combo beat, and on CS-04 only while `dest.ready` (a skip cannot outrun streaming, so the prompt simply is not there yet). The **skip card**: the cutscene's named final frame cut to and held 1.5 s with its card while the bars go out, so a skip never shows a black screen. **Photo mode's camera limits**, which §2.11 records rather than sets. The closing card of CS-10 uses the title's Lora treatment |
| **dungeons.md** | Aligned to that file 2026-09-07; it landed after this file's draft and owns the run flow. The **room strip** is the dungeon HUD element (§2.4.13), fed as `{ path: [{ id, visited, cleared, locked, current }], optional: [{ parentId, id, visited, cleared, locked }], keys }` with `Room N/M`, `🔑 N`, `🔒`, `🔒 LOCKED` and `Wave N/M`; the paper map is the map page's dungeon overlay under the canon `Minimap` setting, never a HUD corner. The **room-transition iris is cut** — rooms are contiguous with real doorways. The one **mechanic meter** slot and its `dungeon.meter` shape stay, serving `Channeling... (N more)`; that file names no per-dungeon meters, because every state is read in the world. The quiet **name cards** (name only, 0.8 s in / 2 s hold / 1 s out) for rest and set-piece rooms, listed per dungeon there. The entry card on bosses.md's timeline with `DUNGEON_NAMES` over `<BIOME> DUNGEON`, the `Entering <dungeon name>...` guest line, and the persistent `SHADOW CITADEL — Floor N: <floor name>` banner in the shard. The prompts and cards: `Resume <name> from the hearth?`, the fail card's two actions `Try again` and `Leave` beside the canon `DUNGEON FAILED` / `DUNGEON RETREAT` and `The Stewart Squad couldn't clear <name>...`, the `Rusted Lantern` item card with `It won't stay lit outside. Inside, it's the only thing that does.`, and the ability-room verbs `[SPACE] Smash` / `Shoot` / `Channel` / `Break` from the live binding. District cards for the Rootways, the Witch's Lanterns path, the Observatory approach and the Sunken Pyramid (name only; a line needs a §6 entry); where the canon `citadel` description lands; `room.visOverride` dims the compass pips but never the room strip; lairs have neither strip nor overlay |
| **heroes.md** (addendum, collected by the orchestrator) | none new; the pad map moves the emote wheel to View-hold (§5.3 item 1) and the camera to right-drag plus `Z` / `C` (item 2). **Both applied to `heroes.md` §2.5.3 on 2026-09-07**, with a §6 line there |
...
8. **`dungeons.md` and `cutscenes.md` had not landed. RESOLVED 2026-09-07** in the consistency pass, in both files' favour. From `dungeons.md`: the compass **room strip** is the dungeon HUD element and the paper map moved to the map page's dungeon overlay under the canon `Minimap` setting; the room-transition iris is cut because rooms are contiguous; the mechanic-meter slot stays but carries only `Channeling... (N more)`, since that file names no per-dungeon meters (§2.0, §2.4.1, §2.4.13, §2.7, §5.2). From `cutscenes.md`: the caption contract, the two letterbox presets and the layer order, the skip prompt's 13 px `#EDE3CF` at 45 % armed at 1.0 s with its pad, touch and vote variants, and photo mode's camera limits, which §2.11 now records instead of assuming (§2.4.11, §2.11, §5.2). Both are logged in §6.
