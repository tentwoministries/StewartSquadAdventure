...
- **The Opus lane is fully closed** (2026-09-08): the worktree is deregistered, its stray vite on :5182 stopped, and `StewartSquad-opus/` deleted after checking that every file on disk was already committed. The branch `phase-0.75-opus-experiment` (tip `59da9a6`) stays as the record; check it out in a fresh worktree if the Rootways or the flight is ever wanted.
